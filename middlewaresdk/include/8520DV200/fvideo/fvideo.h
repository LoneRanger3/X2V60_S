...
typedef union ad_type
{
	...
	struct {
		unsigned board:8;	// 板型:		0: 0~3连接lane_B, 4~7连接lane_A
							// 			1: 0~3连接lane_A, 4~7连接lane_B

		unsigned freq:8;	// AD频率:	0: 148.5M, 1: 74.25M
		unsigned mux:8;		// MUX：	0: 2mux, 一个BT656口采两路
							//      1: 4mux, 一个BT656口采四路
		unsigned reserved:8;
	} b;
} AD_TYPE_T;
...
typedef enum
{
	BT656_MUX2 = 0x0,	// 148.5M
	BT656_MUX4 = 0x1,	// 74.25M
} AD_MUX_E;
