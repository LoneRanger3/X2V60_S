...
typedef enum xmH265E_NALU_TYPE_E
{
     ...
     H265E_NALU_VIRP   = 48,						/*VISLICE types*/
     H265E_NALU_BUTT        
} H265E_NALU_TYPE_E;
...
typedef struct xmJPEG_ENC_ATTR_S
{
	...
	ROTATE_E enRotate;
	unsigned int u32Qfactor; /*image quality :[1,99]*/
	unsigned int scaleEnable;
...
typedef struct xmVENC_PARAM_REF_EX_S
{
	...
	XM_U32	rfcEnable;								/*reference frame compression enable/disable*/
	XM_U32	rfwEnable;								/*reference frame wrap enable/disable*/
	XM_U32	bSingleBuffer;							/*whether to use a single stream buffer*/
	XM_U32	bPreReleaseVb;							/* Whether to Pre-release vb frame */
	XM_U32  u32HwCfgSet;							/*Select the preset hardware configuration*/
	XM_U32	reserved[5];
}VENC_PARAM_REF_EX_S;
...
typedef struct xmVENC_CHN_STAT_S
{
    ...
    XM_U32 u32LeftRecvPics;                         /*Number of frames to be received. This member is valid after XM_MPI_VENC_StartRecvPicEx is called.*/
    XM_U32 u32LeftEncPics;                          /*Number of frames to be encoded. This member is valid after XM_MPI_VENC_StartRecvPicEx is called.*/
}VENC_CHN_STAT_S;
...
{
    XM_BOOL bSplitEnable;                           /*slice split enable, XM_TRUE:enable, XM_FALSE:diable, default value:XM_FALSE*/
...
typedef struct xmVENC_PARAM_H264_INTRA_PRED_S
{
    ...
    XM_U32 constrained_intra_pred_flag;            /*default: XM_FALSE, see the H.264 protocol*/
...
typedef struct xmVENC_PARAM_H264_TRANS_S
{
    ...
    XM_BOOL bScalingListValid;                     /* default: XM_FALSE */
...
/* ROI struct */
typedef struct xmVENC_ROI_ATTR_S
{
    XM_U32  u32Index;                              /* Index of an ROI. The system supports indexes ranging [0, VENC_MAX_ROI_NUM)*/
    XM_BOOL bEnable;                               /* Whether to enable this ROI */
    VENC_ROI_TYPE_E enRoiType;                     /* macroblock coding type */
    XM_BOOL bAbsQp;                                /* QP mode of an ROI.XM_FALSE: relative QP.XM_TURE: absolute QP.*/
...
typedef struct xmVENC_PARAM_REF_S
{
    ...
    XM_BOOL      bEnablePred;                      /*Whether some frames at the base layer are referenced by other frames at the base layer. When bEnablePred is XM_FALSE, all frames at the base layer reference IDR frames.*/
} VENC_PARAM_REF_S;
