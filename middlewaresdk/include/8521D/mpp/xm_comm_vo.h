...
typedef enum xmVO_INTF_SYNC_E
{
	...
	VO_OUTPUT_3840x2160_15,
	VO_OUTPUT_3840x2160_60,
	VO_OUTPUT_600x1024_60,
	VO_OUTPUT_800x1280_60,
...
{
	XM_U32   u32Pll;	/*[0-5] 前分频: [8-19]倍频：[20-24]后分频a:[25-26]后分频b:[28-29]后分频c */
...
typedef struct xmVO_VIDEO_LAYER_ATTR_S
{
    ...
    XM_U32 u32DispMode;                 /* Display Mode */
...
typedef struct xmVO_CURSOR_LAYER_ATTR_S
{
    RECT_S stDispRect;                  /* Display resolution */
    PIXEL_FORMAT_E enPixFormat;         /* Pixel format of the video layer */
	XM_U32  u32Effect;                  /*0-7 tran;8-15 tran0;16-23 tran1; 24-27 HUP;28-31VUP */
} VO_CURSOR_LAYER_ATTR_S;

typedef struct xmVO_CURSOR_LAYER_BITMAP_S
{
	XM_VOID *pData;
	XM_U32  u32Size;
}VO_CURSOR_LAYER_BITMAP_S;

typedef struct xmVO_DSI_CMD_S
{
	XM_U8 u8Data[32];
	XM_U8 u8Size;
	XM_U32 u32Time;
...
typedef struct xmVO_DSI_CMD_LIST_S
{
	VO_DSI_CMD_S *pstCmd;
	XM_U32  u32Len;
}VO_DSI_CMD_LIST_S;

typedef enum xmHDMI_ID_E
{
	XM_HDMI_ID_0 = 0,
	XM_HDMI_ID_BUTT
} XM_HDMI_ID_E;

typedef struct xmHDMI_EDID_S
{
	XM_BOOL bEdidValid;		//EDID 信息是否有效
	XM_U32 u32Edidlength;	//EDID 信息数据长度
	XM_U8 u8Edid[512];		//EDID 信息数据
}XM_HDMI_EDID_S;

typedef struct xmHDMI_VSDB_S
{
	XM_U32 max_clk;		//vic 格式最大时钟
	XM_U32 flag;		//vic 格式，1表示成功读取到vic的值，0表示没有读到vic的值
	XM_U32 hdmi_vic[4];	//vic 格式，1：3840*2160p 29.97 30Hz, 16:9，2：3840*2160p 25Hz, 16:9，3:3840*2160p 23.98 24Hz, 16:9,4:4096*2160p 23.98 24Hz, 256:135
}XM_HDMI_VSDB_S;

typedef struct xmHDMI_DET_TIMING_S
{
	XM_U32 height[4];	//详细时序高
	XM_U32 width[4];	//详细时序宽
	XM_U32 clock[4];	//详细时序时钟
}HDMI_DET_TIMING_S;

typedef struct xmHDMI_SINK_CAPABILITY_S
{
	XM_BOOL bConnected;		//设备是否连接
	HDMI_DET_TIMING_S stDetailedTiming;		//支持的详细时序能力集信息
	XM_HDMI_VSDB_S stVsdb;		//vsdb信息数据
}XM_HDMI_SINK_CAPABILITY_S;
